...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            // 数据目录外部化（AppData）+ 旧 _data 一次性迁移
            config::init_data_dir(app.handle());
            // tmp/ 是 App 私有启动即清区，不放任何持久数据
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // 注：文档命令名 hub_unlink，实现名按语义注册为 hub_unlink_skill
            ...
            commands::session_bind,
            commands::session_lookup,
            commands::session_unbind,
        ])
